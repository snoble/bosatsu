...
object RandomVarEvaluation {
  implicit val valueToLitValue: Value => Option[Normalization.LitValue] = {
    case Value.RandomVarValue(description) => None
    case v => Some(Normalization.LitValue(v.asExternal.toAny))
  }
  implicit val valueToStruct: Value => Option[(Int, List[Value])] = {
    case Value.RandomVarValue(description) => None
    case v => {
      val vSum = v.asSum
  ...
  implicit val valueToList: Value => Option[List[Value]] = { value =>
    @tailrec
    def loop(vv: Value, acc: List[Value]): Option[List[Value]] = vv match {
      case Value.RandomVarValue(description) => None
      case v => { 
        val vSum = v.asSum
        vSum.variant match {
...
case class RandomVarEvaluation(packs: PackageMap.Typed[(Declaration, NormalExpressionTag)], externals: Externals) {
  ...
  def eval(ne: NormalExpression, scope: List[Value], extEnv: Map[Identifier, Eval[Value]]): Value = ne match {
    case NormalExpression.App(fn, arg) => {
      ...
      (fnVal, argVal) match {
        case (Value.RandomVarValue(description), _) => Value.RandomVarValue(s"Applied To $argVal" :: description)
        case _ => fnVal.asFn.apply(argVal)
      }
    }
    case NormalExpression.ExternalVar(p, n) => extEnv(n).value
    case NormalExpression.Match(arg, branches) => {
      val argVal = eval(arg, scope, extEnv)
      val branch = branches.toList.collectFirst(Function.unlift( { case (pat, result) =>
      ...
      branch match {
        case Some((_, patEnv, result)) => ((patEnv.size - 1) to 0 by -1).map(patEnv.get(_).get).foldLeft(eval(result, scope, extEnv)) {case (fn, arg) => fn.asFn(arg) }
        case None => Value.RandomVarValue(List(s"Match on $argVal"))
      }
    }
